...
class DUET(DeepForecastingModelBase):
    ...
    def _process(self, input, target, input_mark, target_mark):
        ...
        # 输出只有c_out维但是框架要求输出维度是data_dim维，所以补齐成data_dim维，后续计算损失时会只取前c_out维进行计算
        ...
        if output.shape[2] < input.shape[2]:  # self.config.data_dim
            ...
            # 补齐到 data_dim
            import torch
            padding = torch.zeros(batch_size, T, data_dim - output.shape[2], device=output.device)
            output = torch.cat([output, padding], dim=2)
        print(f"补齐后的输出形状{output.shape}")
